    ...
    if (!isset($errors['picFile'])) {
        if(isset($name)){
            ...
            if($result->num_rows > 0){
                ...
                $resultAdd = $link->query("UPDATE drogs SET drg_name='". $_POST['name'] ."', drg_company='". $_POST['company'] ."', drg_price='". $_POST['price'] ."', drg_available='". $_POST['available'] ."', drg_category_id='". $_POST['category'] ."', drg_caption='". $_POST['content'] ."', drg_image='".$fileName."' WHERE drg_id=' ". $_POST['drg_id'] ."'");
                if($link->errno == 0){
...
<div class="section-title">
    <h4 class="mb-4">  ویرایش محصول  </h4>
    <a href="index.php?page=products" class="button btn btn-primary">
        ...
        <div class="mb-3">
            ...
            if ($result_submenu->num_rows > 0) {
                while ($row_submenu = $result_submenu->fetch_assoc()) {
                    if($row_submenu['drg_category_id'] == $rowSelect['drg_category_id']){
                        $selected = "selected";
                    }
                    else{
                        $selected = "";
