...
if(isset($_POST['order'])){
    $order = $_POST['order'];
    if($order == 'desc'){
        $sql = "SELECT * FROM factor WHERE fac_user_id = '".$_SESSION['user_id']."' ORDER BY fac_date DESC";
    }
    elseif($order == 'asc'){
        $sql = "SELECT * FROM factor WHERE fac_user_id = '".$_SESSION['user_id']."' ORDER BY fac_date ASC";
    }
    else{
        echo "error";
    }
