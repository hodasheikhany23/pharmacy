...
if(isset($_POST['st'])){
    $st = $_POST['st'];
    if($st == '4')
        $sql = "SELECT * FROM factor WHERE fac_user_id = '".$_SESSION['user_id']."'";
    else if($st == '3' || $st == '2' || $st == '1'){
        $sql = "SELECT * FROM factor WHERE fac_user_id = '".$_SESSION['user_id']."' AND fac_payment_status = '$st'";
    }
    else{
        echo "error";
    }
    $result_factor = $link->query($sql);
}
