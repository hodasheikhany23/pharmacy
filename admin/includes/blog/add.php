...
if(isset($_POST['submit'])){
    if (isset($_FILES['image']) && isset($_POST['name']) && isset($_POST['tag'])) {
        ...
        if (move_uploaded_file($fileTmpPath, $dest_path)) {
            $sql_insert = "INSERT INTO blog (blg_title, blg_cover, blg_date, blg_tag, blg_status) VALUES ('".$_POST['name']."', '".$dest_path."', '".$time."', '".$_POST['tag']."','".$status."')";
            if($link->query($sql_insert) === TRUE){
                $errors['success'] = "  اطلاعات با موفقیت بارگذاری شد";
            }
            else{
                $errors['failed'] = "خطا رد بارگذاری اطلاعات";
